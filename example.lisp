...
(defparameter pitch 440.0d0)
(defparameter seconds-offset 0.0d0)
(declaim (type double-float pitch seconds-offset))
...
(defun write-sample-float32-be (ptr sample)
  ...
  (setf (cffi:mem-ref ptr :float) (float (the double-float sample) 0.0s0)))
...
(defun write-callback (outstream frame-count-min frame-count-max)
  ...
  (let* ((float-sample-rate (soundio:outstream-sample-rate outstream))
         (seconds-per-frame (/ 1.0d0 float-sample-rate))
         (frames-left frame-count-max))
    (declare (type fixnum float-sample-rate frames-left))
    (declare (type double-float seconds-per-frame))
    (cffi:with-foreign-objects ((areas :pointer)
                                (frame-count :int))
      (loop (setf (cffi:mem-ref frame-count :int) frames-left)
            ...
            (let ((frame-count (cffi:mem-ref frame-count :int))
              ...
              (let* ((layout (cffi:foreign-slot-pointer outstream '(:struct soundio:outstream) 'soundio::layout))
                     (radians-per-second (* pitch 2.0d0 PI)))
